...
{
    if(!(p = buscarNodoArbolBinBusq(p,d,cmp)))
        return 0; ///NO_EXISTE    memcpy(d, (*p)->info, MINIMO(tam, (*p)->tamInfo));
    return 1;
}
...
void mostrarNodosHojaRec(tArbolBinBusq *p, void(*mostrar)(const void *))
{
    if(!*p)
        return ;
    ...
    mostrarNodosHojaRec(&(*p)->izq, mostrar);
    mostrarNodosHojaRec(&(*p)->der, mostrar);
}

void mostrarNodosNoHojaRec(tArbolBinBusq *p, void(*mostrar)(const void *))
{
    if(!*p)
        return ;
    ...
    mostrarNodosNoHojaRec(&(*p)->izq, mostrar);
    mostrarNodosNoHojaRec(&(*p)->der, mostrar);
}

void mostrarNodosHijosIzqRec(tArbolBinBusq *p, void(*mostrar)(const void *))
{
    if(!*p)
        return ;
    ...
    mostrarNodosHijosIzqRec(&(*p)->izq,mostrar);
    mostrarNodosHijosIzqRec(&(*p)->der,mostrar);
}

void eliminarArbolCompleto(tArbolBinBusq *p)
{
    if(!*p)
        return;

    eliminarArbolCompleto(&(*p)->izq);
    eliminarArbolCompleto(&(*p)->der);
...
void podarRamasArbolAPartirDeAltura(tArbolBinBusq *p, unsigned altura)
{
    if(!*p)
        return;

    podarRamasArbolAPartirDeAltura(&(*p)->izq, altura == 0 ? 0 : altura-1);
    podarRamasArbolAPartirDeAltura(&(*p)->der, altura == 0 ? 0 : altura-1);

    if(altura == 0 && ((*p)->izq == NULL && (*p)->der == NULL) )
    {
        free((*p)->info);
        free((*p));
        *p = NULL;
...
void podarRamasArbolAlturaDeterminada(tArbolBinBusq *p, unsigned altura)
{
    if(!*p || altura < 0)
        return ;
    ...
    {
        podarRamasArbolAlturaDeterminada(&(*p)->izq, altura-1);
        podarRamasArbolAlturaDeterminada(&(*p)->der, altura-1);
    }
