...
{
    tArbolBinBusq ar;
    int vec[] = {5,10,15,25,50,55,60,65,75,100,110,125,150,175,200},
...
//    FILE * pf;
    ...
    crearArchivo("ordenado.dat", vec, sizeof(int),tamVec);
    ...
    podarRamasArbolAlturaDeterminada(&ar, 3);
    printf("\n\n");
    recorrerEnOrdenInversoArbolBinBusq(&ar,NULL,imprimirConForma);
//    podarRamasArbolAPartirDeAltura(&ar, 3);
//    eliminarArbolCompleto(&ar);
//    if(!alturaArbolBin(&ar))
//        printf("Arbol eliminado!");

    //mostrarNodosHojaRec(&ar, mostrarElem);
    //mostrarNodosNoHojaRec(&ar, mostrarElem);
    //mostrarNodosHijosIzqRec(&ar, mostrarElem);


    //printf("Cantidad nodos: %d", cantNodosArbolBin(&ar));
...
void mostrarElem(const void * datoCrudo)
{
    int * dato = (int*)datoCrudo;
    printf("%d-", *dato);
}
